...
const HeroSection = () => {
  ...
  return (
    <>
      <div dangerouslySetInnerHTML={{ __html: '<!-- COMPONENT: HeroSection -->' }} />
      <section className="relative h-screen w-full overflow-hidden">
        <img
          src={image.heroWedding}
          alt="Wedding photography by Aleksandar Ničić"
          className={`absolute inset-0 w-full h-full object-cover object-bottom transition-all duration-[2s] ${loaded ? "scale-100 opacity-100" : "scale-110 opacity-0"
            }`}
        ...
        <div className="relative z-10 flex flex-col items-end justify-end h-full pb-20 pr-8 md:pr-16">
          <div
            className={`text-right transition-all duration-1000 delay-500 ${loaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
              }`}
          >
          ...
          <p
            className={`heading-caps text-primary-foreground/80 text-xs mt-8 pl-8 md:pl-0 transition-all duration-1000 delay-1000 ${loaded ? "opacity-100" : "opacity-0"}`}
          >
            {t("hero.tagline")}
          </p>
        </div>
